...
const GetUserRental = () => {
    ...
    const handleShowUser = () => {
        ...
        fetch(`${API_URL}/rental/tentantRental`, options)
            .then((res) => {
                if (res.status === 403) {
                    return res.json()
                        .then((json) => {
    ...
    return (
      <div>
          {!showTable && <button onClick={handleShowUser} className="btn">Show all your rentals</button>}
          {showTable && (
              <>
                  <button onClick={handleHideRentals} className="btn">Hide all your rentals</button>
